...
const AchievementsManagement = () => {
  ...
  const handleLinkToggle = async (e) => {
    ...
    try {
      await fetch(`${getApiBaseUrl()}/achievements/link-status`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch students data
        const studentsResponse = await fetch(`${apiBaseUrl}/students`);
        ...
        const achievementsResponse = await fetch(`${apiBaseUrl}/achievements`);
        ...
        // Fetch link status
        const linkStatusResponse = await fetch(`${apiBaseUrl}/achievements/link-status`);
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} md={6} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, alignItems: 'center' }}>
              ...
              <Button 
                ...
                onClick={() => {
                  const hostname = window.location.hostname;
                  const url = `http://${hostname}:5173/student-achievement-form?active=${linkActive}`;
